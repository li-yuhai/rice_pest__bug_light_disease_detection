<template>
  <view class="container">	 
	   <div class="imgBox">
		   <image class="resImg" :src="originImgBase64" mode="aspectFit"> </image>
	   </div>
		 ...
		 async sendPostRequest(img) {
		      try {
		        const response = await uni.request({
		          url: "/web_bing_detect/predict",   // 将接口统一保存在一个配置文件中 js
		          method: 'POST',
		          header: { 
				...
				// this.predict_info = this.getValueCounts(response[1].data[0].pred_info.output);  // 坐标位置信息保存
				this.class_name = response[1].data[0].class;
				this.prob = response[1].data[0].prob;
			...
			selectImgToBase64() {
			  uni.request({
			    ...
			    success: (res) => {
				  ...
				  this.originImgBase64 = 'data:image/jpeg;base64,' + base64;
